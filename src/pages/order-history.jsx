...
const Order = ({ id, date, status }) => {
  return (
    <li className="grid grid-cols-6 py-2">
      ...
      <p className={`col-span-3 text-right rounded-full py-1 px-4 ${status ==="pending"?"bg-yellow-800 text-white":status==="delivered" ? "bg-green-500":""} ml-auto w-fit @md:col-span-2`}>
      {status}
      </p>
    </li>
  );
